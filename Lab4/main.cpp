...
void taskA1()
{
    ...
    double result = (sqrt(pow(x, 2) + 7))/(2 * x) - pow(x, 2) + (sin(alfa))/(abs(pow(x, 3)));
...
void task8()
{
    ...
    cout << "Start value (binary): " << convertToBinary(value) << " (decimal): " << value << endl;

    value &= ~(1 << 0);
    cout << "After LSB (binary): " << convertToBinary(value) << " (decimal): " << value << endl;

    value |= (1 << 7);
    cout << "After MSB (binary): " << convertToBinary(value) << " (decimal): " << value << endl;

    value |= (1 << 2);
    cout << "After set bit nr 2 (binary): " << convertToBinary(value) << " (decimal): " << value << endl;
    ...
    cout << "Bit 5 and 6 are " << ((fifthBitSet && sixthBitSet) ? "set" : "not set") << endl;

    value ^= (1 << 3);
    cout << "After bit 3 inversion (binary): " << convertToBinary(value) << " (decimal): " << value << endl;

    value ^= (1 << 3);
    cout << "After bit 3 inversion again nr 3 (binary): " << convertToBinary(value) << " (decimal): " << value << endl;

    value <<= 2;
    cout << "After shifting 2 bits to the left (binary): " << convertToBinary(value) << " (decimal): " << value << endl;
}
...
void task11()
{
    ...
    cout << "Decimal number " << value << " in binary system: " << convertToBinary(value) << endl;
}
...
{
    /*
    - Zachowaj dokładność do 4 miejsca po przecinku z uwzględnieniem standardowej metody zaokrąglania. 
    - Nie wpisuj końcowych zer (np. jeśli wynik to 3,23 to tak wpisz w oknie odpowiedzi ale nie 3,2300)
    - Podczas wpisywania wyniku użyj przecinka do oddzielenia części całkowitej od rzeczywistej wyniku. 
    - Pamiętaj, że większość języków programowania obliczenia funkcji trygonometrycznych wykonuje w radianach! Przelicz zatem miarę stopniową kąta alfa w radiany przed wstawieniem danych do wzoru!
    */
    double x = 4.29383;
    double a = 7.39434;
    double alfa = 45.5;
    ...
    double result = (sqrt((pow(x, 2) - (2 * a)))) / (2 * x) - (sin(alfa)) / (pow(x, 2)) + (tan(alfa)) / (2 * a);
